...
public class AreaSubController {
    ...
    public List<br.com.rotafuturo.carreiras.dto.AreaSubDTO> search(@org.springframework.web.bind.annotation.RequestParam("search") String search) {
        return areaSubRepository.findAll().stream()
            .filter(a -> a.getAreasDescricao() != null && a.getAreasDescricao().toLowerCase().contains(search.toLowerCase()))
            .map(areaSubService::toDTO)
            .toList();
    }
    @Autowired
    ...
    public ResponseEntity<br.com.rotafuturo.carreiras.dto.AreaSubDTO> getById(@PathVariable Integer id) {
        ...
        return areaSub.map(a -> ResponseEntity.ok(areaSubService.toDTO(a))).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<br.com.rotafuturo.carreiras.dto.AreaSubDTO> create(@RequestBody br.com.rotafuturo.carreiras.dto.AreaSubDTO areaSubDTO) {
